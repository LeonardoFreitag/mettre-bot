...
class Connection {
    ...
    static disconnect(req, res) {
        if (connections[req.params.id]) {
            ...
            res.status(200).json({ id: req.params.id, status: true, message: 'Conexão desconectada com sucesso' });
        } else {
            res.status(200).json({ id: req.params.id, status: false, message: 'Conexão inválida' });
        }
    }
...
module.exports = {
    Connection, ConnectionStatus, ConnectionStatusMessage
}